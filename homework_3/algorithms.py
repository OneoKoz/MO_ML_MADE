...
def gd(filename, x_init, A, y, gamma,
    ...
    for it in range(S):
        ...
        g_k = logreg_grad_plus_lasso(x, [A, y, l2, sparse, l1])
        x -= gamma * g_k
        num_of_data_passes += m
...
def FISTA(filename, x_init, A, y, L,
    ...
    for it in range(S):
        ...
        g_k = logreg_grad(yk, [A, y, mu, sparse])
        xk1 = prox_R(yk - g_k / L, l1 / L)
        tk1 = (1 + np.sqrt(1 + 4 * tk ** 2)) / 2
        yk = xk1 + ((np.sqrt(L/mu) - 1) / (np.sqrt(L/mu) + 1)) * (xk1 - xk)
        xk = xk1
        tk = tk1
        num_of_data_passes += m
...
def prox_gd(filename, x_init, A, y, gamma,
    ...
    for it in range(S):
        ...
        # ваш код здесь
        g_k = logreg_grad(x, [A, y, l2, sparse])
        x = prox_R(x - g_k * gamma, l1 * gamma)
        num_of_data_passes += m
...
def sgd_decr_stepsize(filename, x_init, A, y, gamma_schedule,
    ...
    for it in range(int(S * m / batch_size)):
        ...
        # ваш код здесь
        if number_of_decreases >= decr_period * c:
            gamma *= decr_coeff
            c += 1
        ...
        number_of_decreases += batch_size / m
        g_k = logreg_grad(x, [A_for_batch[batch_ind], y[batch_ind], l2, sparse_stoch])
        x = prox_R(x - gamma * g_k, gamma * l1)

        num_of_data_passes += 2 * batch_size / m
        if ((it + 1) % save_info_period == 0):
            its = np.append(its, it + 1)
            tim = np.append(tim, time.time() - t_start)
            data_passes = np.append(data_passes, num_of_data_passes)
...
def svrg(filename, x_init, A, y, gamma,
    ...
    A_full = A if sparse_full else A.toarray()
    ...
    for s in range(S):
        ...
        w_grad = logreg_grad(w, [A_full, y, l2, sparse_full])
        ...
        for it in range(M):
            ...
            # ваш код здесь
            g_k = logreg_grad(x, [A_for_batch[batch_ind], y[batch_ind], l2, sparse_stoch]) - logreg_grad(w, [
                A_for_batch[batch_ind], y[batch_ind], l2, sparse_stoch]) + w_grad
            x = prox_R(x - g_k * gamma, l1 * gamma)
